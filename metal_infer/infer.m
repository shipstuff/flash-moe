...
// IEEE 754 float16 conversion (for fp16 KV cache)
...
typedef struct {
    ...
    id<MTLBuffer> buf_kv_k[NUM_FULL_ATTN_LAYERS];  // K cache per full-attn layer (float32)
    id<MTLBuffer> buf_kv_v[NUM_FULL_ATTN_LAYERS];  // V cache per full-attn layer (float32)
    id<MTLBuffer> buf_kv_k_f16[NUM_FULL_ATTN_LAYERS]; // K cache fp16 (for fused flash-attn)
    id<MTLBuffer> buf_kv_v_f16[NUM_FULL_ATTN_LAYERS]; // V cache fp16 (for fused flash-attn)
...
static MetalCtx *metal_setup(void) {
    ...
    // GPU attention buffers — TQ_KV=1: compressed KV, TQ_KV=0: float KV
    {
        ...
        if (ctx->use_tq_kv) {
            ...
            // Standard float KV cache (kept for CPU attention fallback at kv_len < 32)
            for (int i = 0; i < NUM_FULL_ATTN_LAYERS; i++) {
            ...
            // fp16 KV cache for fused flash-attention (halves bandwidth at long contexts)
            if (ctx->fused_flash_attn_pipe) {
                size_t kv_cache_f16_size = (size_t)GPU_KV_SEQ * NUM_KV_HEADS * HEAD_DIM * sizeof(uint16_t);
                for (int i = 0; i < NUM_FULL_ATTN_LAYERS; i++) {
                    ctx->buf_kv_k_f16[i] = [ctx->device newBufferWithLength:kv_cache_f16_size
                                                                    options:MTLResourceStorageModeShared];
                    ctx->buf_kv_v_f16[i] = [ctx->device newBufferWithLength:kv_cache_f16_size
                                                                    options:MTLResourceStorageModeShared];
                }
                printf("[metal] FP16 KV: %d layers x %.1f MB/layer (fused flash-attn)\n",
                       NUM_FULL_ATTN_LAYERS, (double)(kv_cache_f16_size * 2) / 1e6);
            }
...
static void gpu_encode_batch_matvec(
    ...
    // Share a single compute encoder across specs that use the same pipeline.
    // Each spec writes to an independent batch_out slot (no data dependencies),
    // so dispatches within the same encoder can execute in parallel on the GPU.
    // This eliminates N-1 encoder creation/destruction overheads per batch.
    id<MTLComputeCommandEncoder> enc = nil;
    int cur_v3 = -1;  // track which pipeline is currently bound
    ...
    for (int i = 0; i < num_specs; i++) {
        ...
        // Create new encoder or switch pipeline if needed
        if (!enc || use_v3 != cur_v3) {
...
static void fused_layer_forward(
    ...
    int fullattn_fused = 0;   // set to 1 if full-attn CMD1+CMD2 fused (norm+RoPE+attn+o_proj+residual+routing)
    ...
    if (prev_gpu_combined && g_metal && g_metal->wf_buf && num_attn_specs > 0) {
        ...
        if (is_full && !can_gpu_linear && prev_gpu_combined && num_attn_specs == 3 &&
            ...
            lc->gate_w && lc->sg_w && lc->su_w && lc->seg_w) {

            int fa_idx = (layer_idx + 1) / FULL_ATTN_INTERVAL - 1;
            int kv_dim_l = NUM_KV_HEADS * HEAD_DIM;
            int q_dim_l = NUM_ATTN_HEADS * HEAD_DIM;

            // Blit: buf_moe_hidden → buf_residual (GPU-side residual copy)
            {
            ...
            // Enc FA1: fullattn_norm_rope_kv (Q/K norm + RoPE + KV cache write + Q/gate split)
            {
                NSUInteger qn_off = (NSUInteger)((const char *)lc->q_norm_w - (const char *)[g_metal->wf_buf contents]);
                NSUInteger kn_off = (NSUInteger)((const char *)lc->k_norm_w - (const char *)[g_metal->wf_buf contents]);
                uint32_t nqh = NUM_ATTN_HEADS, nkvh = NUM_KV_HEADS, hd = HEAD_DIM;
                uint32_t rd = ROTARY_DIM;
                float theta = ROPE_THETA;
                uint32_t pos32 = (uint32_t)pos;
                uint32_t cp = (uint32_t)kv->len;  // cache_pos = kv->len (before increment)
                uint32_t kvd = (uint32_t)kv_dim_l;
                float norm_eps = RMS_NORM_EPS;

                id<MTLComputeCommandEncoder> enc = [cmd1 computeCommandEncoder];
                [enc setComputePipelineState:g_metal->fullattn_norm_rope_pipe];
                [enc setBuffer:g_metal->batch_out[0]         offset:0      atIndex:0];  // q_proj
                [enc setBuffer:g_metal->batch_out[1]         offset:0      atIndex:1];  // k_proj
                [enc setBuffer:g_metal->batch_out[2]         offset:0      atIndex:2];  // v_proj
                [enc setBuffer:g_metal->wf_buf               offset:qn_off atIndex:3];  // q_norm_w bf16
            ...
            if (tq_fullattn) {
                ...
                [enc setBuffer:g_metal->buf_kv_v_f16[fa_idx]  offset:0 atIndex:3];
                ...
                // Fallback: 4-kernel attention pipeline
                uint32_t sl = (uint32_t)(kv->len + 1);
                ...
                [enc setBytes:&sl length:4 atIndex:9];
                [enc dispatchThreadgroups:MTLSizeMake(sl * NUM_ATTN_HEADS, 1, 1) threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
            ...
            // o_proj (reads buf_attn_out, writes buf_output)
            {
                ...
                id<MTLComputeCommandEncoder> enc = [cmd1 computeCommandEncoder];
                uint32_t oo = HIDDEN_DIM, oi = (uint32_t)q_dim_l, ogs = GROUP_SIZE;
            ...
            // residual_add + rms_norm (3 dispatches in 1 encoder)
            {
                ...
                [enc setBytes:&dim length:4 atIndex:4]; [enc setBytes:&feps length:4 atIndex:5];
                [enc dispatchThreadgroups:MTLSizeMake((dim+255)/256, 1, 1) threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
                [enc endEncoding];
            }
            {
                static float s_fused_gate_score = 0;
                BatchMatvecSpec moe_specs[4] = {
                    { lc->gate_w, lc->gate_s, lc->gate_b, s_gate_scores, (uint32_t)NUM_EXPERTS, HIDDEN_DIM, GROUP_SIZE, 0 },
                    { lc->sg_w, lc->sg_s, lc->sg_b, s_shared_gate, (uint32_t)SHARED_INTERMEDIATE, HIDDEN_DIM, GROUP_SIZE, 1 },
                    { lc->su_w, lc->su_s, lc->su_b, s_shared_up, (uint32_t)SHARED_INTERMEDIATE, HIDDEN_DIM, GROUP_SIZE, 2 },
                    { lc->seg_w, lc->seg_s, lc->seg_b, &s_fused_gate_score, 1, HIDDEN_DIM, GROUP_SIZE, 3 },
                };
                gpu_encode_batch_matvec(g_metal, cmd1, moe_specs, 4);
            }
        ...
        if (can_gpu_linear && num_attn_specs == 4) {
            ...
            // Enc L2-L5: rms_norm_qk + decay_beta + delta_net_step + gated_rms_norm
            // Consolidated into single encoder (sequential execution, data-dependent)
            {
            ...
            // ---- CMD1+CMD2 FUSION for linear layers ----
            // Encode CMD2 work (o_proj + residual + norm + routing) INTO cmd1,
            // eliminating a separate cmd_fused command buffer. This applies the
            // ANE super-block fusion insight: fewer dispatch boundaries = less
            // Metal overhead (~0.2 ms saved per linear layer × 45 layers = 9 ms).
            //
            // The residual is in buf_moe_hidden (from previous CMD3, which is
            // on the same serial queue → already completed). GPU blit copies it
            // to buf_residual so the residual_add encoder can read it.
            if (prev_gpu_combined &&
                ...
                int oproj_in_dim_fused = LINEAR_TOTAL_VALUE;
                static float s_shared_gate_score_fused = 0;
                ...
                // o_proj + residual_add + rms_norm (4 dispatches in 1 encoder)
                {
                    ...
                    NSUInteger norm_off = (NSUInteger)((const char *)lc->post_attn_norm_w -
                                                       (const char *)[g_metal->wf_buf contents]);
                    uint32_t o_out = HIDDEN_DIM, o_in = (uint32_t)oproj_in_dim_fused, o_gs = GROUP_SIZE;
                ...
                // routing + shared expert projections (read buf_input)
                // Use static scratch for output (same as the cmd_fused path uses)
                {
                    BatchMatvecSpec moe_specs[4] = {
                        { lc->gate_w, lc->gate_s, lc->gate_b, s_gate_scores,        (uint32_t)NUM_EXPERTS,        HIDDEN_DIM, GROUP_SIZE, 0 },
                        { lc->sg_w,   lc->sg_s,   lc->sg_b,   s_shared_gate,         (uint32_t)SHARED_INTERMEDIATE, HIDDEN_DIM, GROUP_SIZE, 1 },
                        { lc->su_w,   lc->su_s,   lc->su_b,   s_shared_up,           (uint32_t)SHARED_INTERMEDIATE, HIDDEN_DIM, GROUP_SIZE, 2 },
                        { lc->seg_w,  lc->seg_s,  lc->seg_b,  &s_shared_gate_score_fused, 1,                       HIDDEN_DIM, GROUP_SIZE, 3 },
                    };
                    gpu_encode_batch_matvec(g_metal, cmd1, moe_specs, 4);
                }

                // Mark that CMD1 now includes CMD2 work — skip cmd_fused later
                gpu_linear_attn = 2;  // 2 = fully fused (CMD1 has CMD2 too)
            }
        }
        ...
        if (!gpu_linear_attn && !fullattn_fused) {
            gpu_flush_batch_results(g_metal, attn_specs, num_attn_specs);
        }
        if (fullattn_fused && kv) kv->len++;  // GPU wrote KV cache in CMD1; CPU len tracks it
    ...
    if (fullattn_fused) {
        // ---- Full attention already fused into CMD1 — skip CPU attention ----
        // attn_out_for_oproj stays NULL; readback section handles fullattn_fused
    } else if (is_full) {
        ...
        if (g_metal && g_metal->attn_scores_pipe && !g_metal->use_tq_kv &&
            ...
            // Also write fp16 KV cache for fused flash-attention
            if (g_metal->buf_kv_k_f16[fa_idx]) {
                uint16_t *k_f16 = (uint16_t *)[g_metal->buf_kv_k_f16[fa_idx] contents] + cache_pos * kv_dim;
                uint16_t *v_f16 = (uint16_t *)[g_metal->buf_kv_v_f16[fa_idx] contents] + cache_pos * kv_dim;
                for (int i = 0; i < kv_dim; i++) {
                    k_f16[i] = f32_to_f16(k_out[i]);
                    v_f16[i] = f32_to_f16(v_out[i]);
                }
            }
        }
    ...
    if (fullattn_fused) {
        // ---- CMD1 already fused full-attn: norm+RoPE+attention+o_proj+residual+routing ----
        // CMD1 was committed and waited on in the fast-path. GPU buffers have results.
        if (g_timing_enabled) { t0 = now_ms(); }
        gpu_flush_batch_results(g_metal,
            (BatchMatvecSpec[]){{ gate_w, gate_s, gate_b, gate_scores, (uint32_t)NUM_EXPERTS, HIDDEN_DIM, GROUP_SIZE, 0 },
                                { sgw,    sgs,    sgb,    shared_gate, (uint32_t)SHARED_INTERMEDIATE, HIDDEN_DIM, GROUP_SIZE, 1 },
                                { suw,    sus,    sub,    shared_up,   (uint32_t)SHARED_INTERMEDIATE, HIDDEN_DIM, GROUP_SIZE, 2 },
                                { seg_w,  seg_s,  seg_b,  &shared_gate_score, 1, HIDDEN_DIM, GROUP_SIZE, 3 }}, 4);
    ...
    } else if (gpu_linear_attn == 2) {
        // ---- CMD1+CMD2 already fused for this linear layer ----
        // Just read back the results from GPU buffers to CPU scratch.
        gpu_flush_batch_results(g_metal,
            (BatchMatvecSpec[]){{ lc->gate_w, lc->gate_s, lc->gate_b, gate_scores, (uint32_t)NUM_EXPERTS, HIDDEN_DIM, GROUP_SIZE, 0 },
                                { lc->sg_w, lc->sg_s, lc->sg_b, shared_gate, (uint32_t)SHARED_INTERMEDIATE, HIDDEN_DIM, GROUP_SIZE, 1 },
                                { lc->su_w, lc->su_s, lc->su_b, shared_up, (uint32_t)SHARED_INTERMEDIATE, HIDDEN_DIM, GROUP_SIZE, 2 },
                                { lc->seg_w, lc->seg_s, lc->seg_b, &shared_gate_score, 1, HIDDEN_DIM, GROUP_SIZE, 3 }}, 4);
        memcpy(h_mid, [g_metal->buf_h_mid contents], HIDDEN_DIM * sizeof(float));
        memcpy(h_post, [g_metal->buf_input contents], HIDDEN_DIM * sizeof(float));
        memcpy(hidden, h_mid, HIDDEN_DIM * sizeof(float));
    } else if ((attn_out_for_oproj || gpu_attn_fuse) && oproj_w && oproj_s && oproj_b &&
        ...
        if (gpu_attn_fuse) {
            ...
            if (g_metal->use_tq_kv && g_metal->tq_fused_attn_pipe) {
                ...
                [enc dispatchThreadgroups:MTLSizeMake(NUM_ATTN_HEADS, 1, 1)
                    threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
                [enc endEncoding];
                // Skip the separate sigmoid_gate dispatch — it's fused in
            } else {
            ...
            // Enc A3: attn_values_batched
            {
                ...
                [enc dispatchThreadgroups:MTLSizeMake(tgs, 1, 1)
                    threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
                [enc endEncoding];
                }  // end else (fallback non-TurboQuant path)
            }
            // Enc A4: sigmoid_gate (skip when fused flash-attention already applied it)
...
// ============================================================================
// Phase 2a: Multi-slot linear-attention layer
//
// Parallelize PROJECTIONS (90% of per-layer compute) across T slots while
// keeping the recurrence SEQUENTIAL (the delta-rule recurrence is nonlinear
// and can't be efficiently parallel-scanned — see
// docs/2026-04-12-parallel-scan-analysis.md).
//
// Flow:
//   1. CPU: input_norm for each slot → buf_input_slot[t]
//   2. GPU: dispatch T CMD1 projection matvecs (qkv, z, b, a) in parallel
//   3. CPU: wait all T CMD1s
//   4. For each token t SEQUENTIALLY on the main queue:
//      a. Copy projections from batch_out_slot[t][0..3] → batch_out[0..3]
//      b. GPU: encode conv1d + rms_norm_qk + compute_decay_beta +
//              delta_net_step + gated_rms_norm → commit + wait
//      c. Copy gated output to batch_out_slot[t][6] for per-slot CMD2
//   5. GPU: dispatch T CMD2 (o_proj + residual + norm + routing) in parallel
//   6. CPU: wait all T CMD2s, read back hidden state
//
// Returns with hidden_batch updated to h_mid per slot. buf_input_slot[t]
// contains h_post, batch_out_slot[t][3] contains gate_scores.
// ============================================================================

// Helper: encode the 5 recurrence kernels (L1-L5) onto a command buffer.
// These read from batch_out[0..3] (the projection results which must be
// in the shared/slot-0 batch_out buffers) and the per-layer state.
static void encode_linear_recurrence_kernels(
    id<MTLCommandBuffer> cmd,
    LayerWeightCache *lc,
    ...
    // Enc L4: delta_net_step (the recurrence — reads/writes persistent state)
    {
        uint32_t khpv = LINEAR_NUM_V_HEADS / LINEAR_NUM_K_HEADS;
        id<MTLComputeCommandEncoder> enc = [cmd computeCommandEncoder];
...
static void multi_slot_linear_attn_layer(
    ...
    LayerWeightCache *lc = &layer_cache[layer_idx];
    int linear_layer_idx = layer_idx - (layer_idx + 1) / FULL_ATTN_INTERVAL;
    if (linear_layer_idx < 0 || linear_layer_idx >= NUM_LINEAR_LAYERS) return;
    if (!lc->qkv_w || !lc->z_w || !lc->b_w || !lc->a_w) return;
    if (!g_metal->delta_net_step || !g_metal->conv1d_step) return;
    ...
    for (int t = 0; t < T; t++) {
        ...
        #define ENC_LA_MV(W, S_, B, OUT_SLOT, OUT_DIM) do { \
            ...
            [enc setBuffer:ctx->batch_out_slot[t][(OUT_SLOT)] offset:0 atIndex:4]; \
            uint32_t _od = (OUT_DIM); uint32_t _id = HIDDEN_DIM; uint32_t _gs = GROUP_SIZE; \
            [enc setBytes:&_od length:4 atIndex:5]; [enc setBytes:&_id length:4 atIndex:6]; \
            [enc setBytes:&_gs length:4 atIndex:7]; \
            if (use_v3) { [enc dispatchThreadgroups:MTLSizeMake((_od+7)/8,1,1) threadsPerThreadgroup:MTLSizeMake(256,1,1)]; } \
        ...
        ENC_LA_MV(lc->qkv_w, lc->qkv_s, lc->qkv_b, 0, qkv_dim);
        ENC_LA_MV(lc->z_w,   lc->z_s,   lc->z_b,   1, z_dim);
        ENC_LA_MV(lc->b_w,   lc->b_s,   lc->b_b,   2, LINEAR_NUM_V_HEADS);
        ENC_LA_MV(lc->a_w,   lc->a_s,   lc->a_b,   3, LINEAR_NUM_V_HEADS);
        #undef ENC_LA_MV
    ...
    for (int t = 0; t < T; t++) {
        ...
        encode_linear_recurrence_kernels(cmd_rec, lc, linear_layer_idx);
        [cmd_rec commit];
        [cmd_rec waitUntilCompleted];
    ...
    // The o_proj for linear attention reads from batch_out[6] (gated output)
    // and writes to buf_output. Then residual_add, rms_norm, routing projections.
    id<MTLCommandBuffer> cmd2s[MAX_MULTI_T];
    for (int t = 0; t < T; t++) {
        cmd2s[t] = [ctx->queue_slot[t] commandBuffer];

        // o_proj matvec (linear-attn out_proj: value_dim → hidden_size)
        if (lc->out_proj_w && lc->out_proj_s && lc->out_proj_b) {
            NSUInteger w_off = (NSUInteger)((const char *)lc->out_proj_w - (const char *)[ctx->wf_buf contents]);
            NSUInteger s_off = (NSUInteger)((const char *)lc->out_proj_s - (const char *)[ctx->wf_buf contents]);
            NSUInteger b_off = (NSUInteger)((const char *)lc->out_proj_b - (const char *)[ctx->wf_buf contents]);
            id<MTLComputeCommandEncoder> enc = [cmd2s[t] computeCommandEncoder];
            uint32_t oo = HIDDEN_DIM, oi = LINEAR_TOTAL_VALUE, ogs = GROUP_SIZE;
            [enc setComputePipelineState:ctx->matvec_fast];
            [enc setBuffer:ctx->wf_buf offset:w_off atIndex:0];
            [enc setBuffer:ctx->wf_buf offset:s_off atIndex:1];
            [enc setBuffer:ctx->wf_buf offset:b_off atIndex:2];
            [enc setBuffer:ctx->batch_out_slot[t][6] offset:0 atIndex:3]; // per-slot gated output
            [enc setBuffer:ctx->buf_output_slot[t] offset:0 atIndex:4];
            [enc setBytes:&oo length:4 atIndex:5]; [enc setBytes:&oi length:4 atIndex:6];
            [enc setBytes:&ogs length:4 atIndex:7];
            [enc dispatchThreadgroups:MTLSizeMake(oo, 1, 1) threadsPerThreadgroup:MTLSizeMake(64, 1, 1)];
            [enc endEncoding];
        ...
        // residual_add (buf_output_slot[t] + buf_residual_slot[t] → buf_h_mid_slot[t])
        {
            id<MTLComputeCommandEncoder> enc = [cmd2s[t] computeCommandEncoder];
            uint32_t dim = HIDDEN_DIM;
            [enc setComputePipelineState:ctx->residual_add];
            [enc setBuffer:ctx->buf_residual_slot[t] offset:0 atIndex:0];
            [enc setBuffer:ctx->buf_output_slot[t] offset:0 atIndex:1];
            [enc setBuffer:ctx->buf_h_mid_slot[t] offset:0 atIndex:2];
            [enc setBytes:&dim length:4 atIndex:3];
            [enc dispatchThreadgroups:MTLSizeMake((dim+255)/256, 1, 1) threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
            [enc endEncoding];
        }
        // rms_norm_sum + rms_norm_apply → buf_input_slot[t] (h_post for routing)
        {
            id<MTLComputeCommandEncoder> enc = [cmd2s[t] computeCommandEncoder];
            uint32_t dim = HIDDEN_DIM;
            [enc setComputePipelineState:ctx->rms_norm_sum];
        ...
        // routing + shared expert projections (read from buf_input_slot[t])
        #define ENC_RT_MV(W, S_, B, OUT_BUF, OUT_DIM) do { \
            NSUInteger w_off = (NSUInteger)((const char *)(W)  - (const char *)[ctx->wf_buf contents]); \
            NSUInteger s_off = (NSUInteger)((const char *)(S_) - (const char *)[ctx->wf_buf contents]); \
            NSUInteger b_off = (NSUInteger)((const char *)(B)  - (const char *)[ctx->wf_buf contents]); \
            id<MTLComputeCommandEncoder> enc = [cmd2s[t] computeCommandEncoder]; \
        ...
        ENC_RT_MV(lc->gate_w, lc->gate_s, lc->gate_b, ctx->batch_out_slot[t][3], NUM_EXPERTS);
        ENC_RT_MV(lc->sg_w,   lc->sg_s,   lc->sg_b,   ctx->buf_shared_gate_slot[t], SHARED_INTERMEDIATE);
        ENC_RT_MV(lc->su_w,   lc->su_s,   lc->su_b,   ctx->buf_shared_up_slot[t],   SHARED_INTERMEDIATE);
        ENC_RT_MV(lc->seg_w,  lc->seg_s,  lc->seg_b,  ctx->buf_shared_out_slot[t],   1);
        #undef ENC_RT_MV
    ...
    for (int t = 0; t < T; t++) {
        float *hidden_t = hidden_batch + (size_t)t * HIDDEN_DIM;
        memcpy(hidden_t, [ctx->buf_h_mid_slot[t] contents], HIDDEN_DIM * sizeof(float));
    }
}

// ============================================================================
// ============================================================================
// Phase 2d: BATCHED expert dispatch across T tokens using dispatch_experts_sparse.
// Loads each unique expert ONCE across all T tokens' routing decisions.
// Then dispatches CMD3 (combine + norm) ASYNC on per-slot queues.
//
// This replaces the per-token serial dispatch loop with:
// 1. Collect all T tokens' routing decisions from per-slot buffers
// 2. Collect all T tokens' h_post from buf_input_slot[t]
// 3. Call dispatch_experts_sparse (loads each unique expert once, applies to all)
// 4. Per-token: shared expert (SwiGLU + down_proj) + combine + norm
// 5. Dispatch CMD3 async on queue_slot[t]
// ============================================================================
static void multi_slot_expert_dispatch_batched(
    int layer_idx,
    float *hidden_batch,   // [T * HIDDEN_DIM] in/out — h_mid on input
    int T,
    int K,
    int packed_fd,
    const void *mmap_base
    ...
    // Step 1: Collect T tokens' routing results from per-slot buffers
    int expert_indices_batch[MAX_MULTI_T * MAX_K];
    float gate_weights_batch[MAX_MULTI_T * MAX_K];
    float shared_gate_scores[MAX_MULTI_T];
    float h_post_batch[MAX_MULTI_T * HIDDEN_DIM];

    for (int t = 0; t < T; t++) {
        float gate_scores[NUM_EXPERTS];
        memcpy(gate_scores, [ctx->batch_out_slot[t][3] contents], NUM_EXPERTS * sizeof(float));
        cpu_softmax(gate_scores, NUM_EXPERTS);

        int *ei = expert_indices_batch + t * actual_K;
        float *ew = gate_weights_batch + t * actual_K;
        cpu_topk(gate_scores, NUM_EXPERTS, actual_K, ei, ew);
        cpu_normalize_weights(ew, actual_K);

        // Read h_post from buf_input_slot[t]
        memcpy(h_post_batch + (size_t)t * HIDDEN_DIM,
               [ctx->buf_input_slot[t] contents], HIDDEN_DIM * sizeof(float));

        // Read shared_expert_gate_score
        memcpy(&shared_gate_scores[t], [ctx->buf_shared_out_slot[t] contents], sizeof(float));
    }

    // Step 2: Batched expert dispatch — loads each unique expert ONCE
    float expert_out_batch[MAX_MULTI_T * HIDDEN_DIM];
    dispatch_experts_sparse(T, actual_K,
                            expert_indices_batch, gate_weights_batch,
                            h_post_batch, expert_out_batch,
                            NULL, 0,  // not synthetic
                            layer_idx, packed_fd);

    // Step 3: Per-token shared expert + combine + async CMD3
    for (int t = 0; t < T; t++) {
        float *hidden_t = hidden_batch + (size_t)t * HIDDEN_DIM;
        ...
        // GPU combine: expert_out already accumulated in slot [0] with weight 1.0
        // Use moe_combine_residual with K=1 and weight[0]=1.0, shared_gate=score
        ...
        if (gpu_combine) {
            ...
            memset(params, 0, 10 * sizeof(float));
            params[0] = 1.0f;  // expert_out_t already pre-weighted
            params[8] = shared_gate_scores[t];
            ...
            // Enc C1: combine
            {
                ...
                uint32_t dim = HIDDEN_DIM;
                uint32_t k_val = 1;  // just one pre-accumulated expert output
                [enc setBytes:&dim   length:4 atIndex:12];
                [enc setBytes:&k_val length:4 atIndex:13];
                [enc dispatchThreadgroups:MTLSizeMake((dim + 255) / 256, 1, 1)
                    threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
            ...
            // Enc C3: rms_norm_apply → buf_input_slot[t]
            {
                uint16_t *next_norm_w = layer_cache[layer_idx + 1].input_norm_w;
        ...
        // Commit ASYNC on per-slot queue
        [cmd3 commit];
        ...
        g_deferred.hidden = hidden_t;
        g_deferred.layer_idx = layer_idx;
        g_current_slot = 0;
    }
}

// Phase 2c: Per-token expert dispatch after multi-slot layer returns.
//
// Reads routing results from per-slot buffers (batch_out_slot[t][3] = gate_scores,
// buf_shared_gate_slot[t] = shared gate, buf_shared_up_slot[t] = shared up,
// buf_shared_out_slot[t] = shared_expert_gate_score).
// Dispatches K experts serially using slot-0 Metal buffers.
// Uses the GPU combine path (moe_combine_residual) for efficiency.
// ============================================================================
static void multi_slot_expert_dispatch_token(
    int layer_idx,
    float *hidden,        // [HIDDEN_DIM] in/out — h_mid on input, combined on output
    int t,                // which slot's results to use
    int K,
    int packed_fd,
    const void *mmap_base
) {
    ...
    // Read gate_scores from per-slot buffer
    float gate_scores[NUM_EXPERTS];
    memcpy(gate_scores, [ctx->batch_out_slot[t][3] contents], NUM_EXPERTS * sizeof(float));
    ...
    // Read shared_expert_gate_score (1 float from buf_shared_out_slot[t])
    float shared_gate_score;
    memcpy(&shared_gate_score, [ctx->buf_shared_out_slot[t] contents], sizeof(float));

    // Pread K experts into buf_multi_expert_data[k]
    size_t esz = active_expert_size();
    int valid[MAX_K];
    ...
    if (g_malloc_cache) {
        ...
        // No cache: parallel pread into buf_multi_expert_data[k]
        InferPreadTask tasks[MAX_K];
        memset(tasks, 0, sizeof(tasks));
        for (int k = 0; k < actual_K; k++) {
            ...
            tasks[k].size = esz;
            tasks[k].mmap_base = mmap_base;
            expert_bufs[k] = ctx->buf_multi_expert_data[k];
        }
        io_pool_dispatch(tasks, actual_K);
        for (int k = 0; k < actual_K; k++) {
            valid[k] = (tasks[k].result == (ssize_t)esz);
...
// ============================================================================
// Phase 2c: Full multi-slot prefill driver
//
// Processes T prefill tokens through all 60 layers using:
//   - multi_slot_full_attn_layer for full-attention layers (GPU parallel dispatch)
//   - multi_slot_linear_attn_layer for linear-attention layers (parallel projections)
//   - multi_slot_expert_dispatch_token per token per layer (serial expert dispatch)
//
// Gated by MULTI_SLOT_PREFILL=1 env var via the multi_slot_prefill_enabled flag.
// Falls back to fused_layer_forward per-token if disabled.
// ============================================================================
static int g_multi_slot_prefill_enabled = -1;  // -1 = not checked, 0 = off, 1 = on

static void multi_slot_prefill_chunk(
    WeightFile *wf,
    float *hidden_chunk,   // [T * HIDDEN_DIM] in/out
    int T,
    KVCache **kv_caches,
    LinearAttnState **layer_states,
    int *layer_fds,
    void **layer_mmaps,
    int K,
    int pos_start
...
int main(int argc, char **argv) {
    ...
    @autoreleasepool {
        ...
        if (pt->count > 1) {
            ...
            if (g_multi_slot_prefill_enabled && batch_prefill_T > 1) {
                // ---- Phase 2c: Multi-slot prefill ----
                // Uses parallel GPU dispatch for projections + serial recurrence
                // for linear-attention + serial expert dispatch per token.
                int T_ms = batch_prefill_T;
                if (T_ms > MAX_MULTI_T) T_ms = MAX_MULTI_T;
                float *hidden_chunk = malloc((size_t)T_ms * HIDDEN_DIM * sizeof(float));
                int processed = 0;
                while (processed < intermediate_count) {
                    int T_now = intermediate_count - processed;
                    if (T_now > T_ms) T_now = T_ms;
                    double t_chunk = now_ms();

                    for (int t = 0; t < T_now; t++) cache_telemetry_note_token();
                    memcpy(hidden_chunk,
                           embed_batch + (size_t)processed * HIDDEN_DIM,
                           (size_t)T_now * HIDDEN_DIM * sizeof(float));

                    multi_slot_prefill_chunk(wf, hidden_chunk, T_now,
                                             kv_caches, layer_states,
                                             layer_fds, (void **)layer_mmaps,
                                             K, pos);
                ...
                free(hidden_chunk);
            } else if (batch_prefill_T > 1) {
                // Chunked prefill: run T tokens together per layer (cold-cache-only legacy).
        ...
        // ---- Release malloc cache before generation ----
        // The 18GB malloc cache competes with the OS page cache for physical memory,
        // causing a 34% generation throughput regression (3.84 vs 5.86 tok/s at 138-tok).
        // Releasing it after prefill gives best-of-both: fast TTFT + full gen speed.
        if (g_malloc_cache) {
            malloc_cache_free(g_malloc_cache);
            g_malloc_cache = NULL;
            printf("[malloc_cache] Released after prefill — OS page cache reclaimed for generation\n");
        }
